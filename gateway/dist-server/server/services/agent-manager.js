...
export function buildTaskEvent(toolName, input) {
    switch (toolName) {
        case 'TaskCreate':
            return {
                action: 'create',
        ...
        case 'TaskUpdate': {
            ...
            if (input.addBlockedBy)
                task.blockedBy = input.addBlockedBy;
            if (input.addBlocks)
                task.blocks = input.addBlocks;
            if (input.owner)
...
export class AgentManager {
    ...
    async *sendMessage(sessionId, content, opts) {
        ...
        const toolState = {
            ...
            appendTaskInput: (chunk) => { taskToolInput += chunk; },
        ...
        try {
            ...
            while (true) {
                ...
                if (!pendingSdkPromise) {
                    pendingSdkPromise = sdkIterator.next().then(result => ({ sdk: true, result }));
                }
