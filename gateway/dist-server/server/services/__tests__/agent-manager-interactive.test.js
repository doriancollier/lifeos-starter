...
describe('AgentManager interactive tools', () => {
    ...
    describe('checkSessionHealth', () => {
        ...
        it('cleans up pending interactions of expired sessions', async () => {
            ...
            const toolApprovalPromise = new Promise((resolve) => {
                ...
                mockedQuery.mockReturnValue(mockAsyncIterable);
                resolve();
            });
    ...
    describe('sendMessage', () => {
        ...
        it('yields text_delta events from SDK stream_events', async () => {
            ...
            expect(textDeltas[0].data.text).toBe('Hello world');
        });
        it('yields tool_call_start and tool_call_end events', async () => {
            ...
            expect(toolStarts[0].data.toolName).toBe('Read');
            expect(toolDeltas).toHaveLength(1);
            expect(toolDeltas[0].data.input).toBe('{"path":"/tmp"}');
        ...
        it('yields tool_result events from tool_use_summary messages', async () => {
            ...
            expect(toolResults[0].data.result).toBe('File contents: hello');
            expect(toolResults[0].data.toolCallId).toBe('tc-1');
            expect(toolResults[1].data.toolCallId).toBe('tc-2');
        });
        it('yields done event at the end of a stream', async () => {
            ...
            expect(doneEvents[0].data.sessionId).toBe('sess-1');
        });
        it('yields error event when SDK throws', async () => {
            ...
            expect(errorEvents[0].data.message).toBe('SDK connection failed');
        ...
        it('yields session_status with model from init message', async () => {
            ...
            expect(statusEvents[0].data.model).toBe('claude-opus-4');
        });
        ...
        it('sets resume on SDK options when session has started', async () => {
            ...
            const secondCallArgs = mockedQuery.mock.calls[1][0];
    ...
    describe('canUseTool integration', () => {
        it('AskUserQuestion pushes question_prompt and submitAnswers resolves it', async () => {
            ...
            const permissionPromise = canUseToolFn('AskUserQuestion', { questions }, { signal: new AbortController().signal, toolUseID: 'tool-q1' });
            ...
            expect(firstEvent.value.data.toolCallId).toBe('tool-q1');
            // Now submit answers
        ...
        it('tool approval in default mode pushes approval_required and approveTool resolves it', async () => {
            ...
            const permissionPromise = canUseToolFn('Write', { file_path: '/tmp/test.txt', content: 'hello' }, { signal: new AbortController().signal, toolUseID: 'tool-w1' });
            const firstEvent = await pullPromise;
            expect(firstEvent.done).toBe(false);
            expect(firstEvent.value.type).toBe('approval_required');
            expect(firstEvent.value.data.toolName).toBe('Write');
            // Approve the tool
        ...
        it('denying a tool approval resolves with deny', async () => {
            ...
            const gen = manager.sendMessage('sess-1', 'do something');
            const pullPromise = gen.next();
            const permissionPromise = canUseToolFn('Bash', { command: 'rm -rf /' }, { signal: new AbortController().signal, toolUseID: 'tool-b1' });
        ...
        it('non-AskUserQuestion tools in bypassPermissions mode are allowed immediately', async () => {
            ...
            const result = await canUseToolFn('Write', { file_path: '/tmp/test.txt' }, { signal: new AbortController().signal, toolUseID: 'tool-w1' });
    ...
    describe('interaction timeout', () => {
        it('question interaction times out after 10 minutes with deny', async () => {
            ...
            gen.next(); // kick off
            await vi.advanceTimersByTimeAsync(0);
            const permissionPromise = canUseToolFn('AskUserQuestion', { questions: [] }, { signal: new AbortController().signal, toolUseID: 'tool-q-timeout' });
        ...
        it('tool approval interaction times out after 10 minutes with deny', async () => {
            ...
            gen.next(); // kick off
            await vi.advanceTimersByTimeAsync(0);
            const permissionPromise = canUseToolFn('Bash', { command: 'ls' }, { signal: new AbortController().signal, toolUseID: 'tool-a-timeout' });
            await vi.advanceTimersByTimeAsync(10 * 60 * 1000 + 1);
            const result = await permissionPromise;
            expect(result).toEqual({
                behavior: 'deny',
...
//# sourceMappingURL=agent-manager-interactive.test.js.map