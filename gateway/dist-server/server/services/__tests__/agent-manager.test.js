...
describe('AgentManager', () => {
    ...
    describe('sendMessage()', () => {
        ...
        it('streams SDK text_delta events', async () => {
            ...
            const doneEvent = events.find(e => e.type === 'done');
            expect(doneEvent).toBeDefined();
            expect(doneEvent.data.sessionId).toBe('s1');
        });
        it('streams tool call events', async () => {
            ...
            expect(deltaEvent.data.input).toBe('{"file":"test.ts"}');
            const endEvent = events.find(e => e.type === 'tool_call_end');
            expect(endEvent).toBeDefined();
            expect(endEvent.data.status).toBe('complete');
        });
        it('handles SDK errors gracefully', async () => {
            ...
            expect(errorEvent.data.message).toBe('API key not found');
...
//# sourceMappingURL=agent-manager.test.js.map