...
The `Transport` interface (`src/shared/transport.ts`) defines 9 methods that cover all client-server communication:
...
All four plugins run in this order during `vite build --config vite.config.obsidian.ts`:

1. `copyManifest()` -- copies `manifest.json` to `dist-obsidian/`
...
### Standalone Web (`vite.config.ts`)

Standard Vite React build. Server compiled separately via `tsc`.

### Obsidian Plugin (`vite.config.obsidian.ts`)
...
This pattern replaces the previous `vi.mock('../../lib/api')` approach, providing better type safety and more explicit test setup.
