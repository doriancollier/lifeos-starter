...
All request/response types are defined as **Zod schemas** in `src/shared/schemas.ts`. This single file is the source of truth for:
...
Types in `src/shared/types.ts` are re-exported from `schemas.ts`, so all existing imports continue to work.

## Adding a New Endpoint

1. **Define schemas** in `src/shared/schemas.ts` for request/response shapes
2. **Register the path** in `src/server/services/openapi-registry.ts` with tags, request schema, and response schemas
...
4. Export the inferred type from `src/shared/types.ts` if needed by client code
