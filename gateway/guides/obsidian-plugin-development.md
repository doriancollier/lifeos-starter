...
The plugin uses a separate Vite build (`vite.config.obsidian.ts`) that outputs a single CJS `main.js` with four custom build plugins for Electron compatibility:
...
| --- | --- | --- |
| `copyManifest()` | closeBundle | Copies `manifest.json` to `dist-obsidian/` |
...
```bash
npm run build:obsidian   # vite build --config vite.config.obsidian.ts
```

### Development Workflow

1. Run `npx vite build --config vite.config.obsidian.ts`
2. The build outputs to `dist-obsidian/` which is symlinked (or hardlinked) into the vault's `.obsidian/plugins/lifeos-copilot/`
...
├── vite.config.obsidian.ts           # Plugin build (4 Electron compat plugins)
└── package.json
```
...
Our plugin is built to `gateway/dist-obsidian/`. The Obsidian vault symlinks to it:

```bash
# Already done for this vault:
ln -s /path/to/gateway/dist-obsidian workspace/.obsidian/plugins/lifeos-copilot
```

The Vite build (`npm run build:obsidian`) outputs all three required files:
...
| "Cannot find module 'obsidian'" | Not externalized in Vite config | Add to `external` in `vite.config.obsidian.ts` rollupOptions |
...
1. Run `npm run build:obsidian` (or `dev:obsidian` for watch mode)
...
| File | Purpose |
| --- | --- |
| `src/plugin/main.ts` | Plugin entry (onload/onunload) |
| `src/plugin/views/CopilotView.tsx` | ItemView — creates services, mounts React |
| `src/plugin/components/ObsidianApp.tsx` | Plugin-specific App (auto-session, compact layout) |
| `src/shared/transport.ts` | Transport interface definition |
| `src/client/lib/direct-transport.ts` | In-process transport wrapping services |
| `src/client/contexts/TransportContext.tsx` | React Context for DI |
| `vite.config.obsidian.ts` | Plugin build config |
