...
```typescript
// src/server/services/agent-manager.ts
...
```typescript
// src/client/hooks/use-chat-session.ts
...
Create a component in `src/client/components/chat/` that:
...
```typescript
// src/client/components/chat/MessageItem.tsx
...
Route-level tests for interactive endpoints mock `agentManager` and verify HTTP status codes and request/response shapes. See `src/server/routes/__tests__/sessions-interactive.test.ts` for examples:
