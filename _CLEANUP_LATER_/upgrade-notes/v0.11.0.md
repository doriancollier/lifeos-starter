...
## Summary
Major architecture overhaul: vault content moved to ``, new infrastructure directories for background tasks, integrations, and multi-channel gateway (future).

## User Action Required

- [ ] Re-open Obsidian and point it to `` folder (not root)
- [ ] Verify your daily notes work: run `/daily:plan`
- [ ] If you have custom scripts referencing vault paths, update them to use ``

## Breaking Changes

### Vault Location Changed
- **Old**: All vault content at repository root (`0-System/`, `4-Daily/`, etc.)
- **New**: All vault content under `` (`0-System/`, `4-Daily/`, etc.)
- **Migration**: Automatic. Obsidian must be pointed at `` folder.

### VAULT_ROOT Environment Variable
- If you set `OBSIDIAN_VAULT_ROOT`, it should now point to ``
...
2. Run `/daily:plan` - daily note should be created in `4-Daily/`
3. Open Obsidian with `` as the vault - all notes should be accessible
4. Check new directories exist: `ls -la` should show integrations/, tasks/, data/, state/, extensions/

## Notes for AI

- If user reports "file not found" errors, check if paths include ``
- If user's Obsidian is empty, guide them to re-open with `` as vault
